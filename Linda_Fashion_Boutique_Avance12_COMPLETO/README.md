...
La base `data/ferreteria.db` se crea/actualiza sin borrar los registros existentes. Los productos se validan con Flask-WTF, se guardan mediante un `INSERT` parametrizado con `?`, se confirma con `commit()` y se recuperan mediante `SELECT` y `fetchall()` para mostrarlos con Jinja2.
